...
# Human3.6M 17-joint skeletal edges. Used by UI-PRMD and our unified binary task.
H36M17_EDGES: tuple[tuple[int, int], ...] = (
    (0, 1), (1, 2), (2, 3),       # Pelvis - RHip - RKnee - RAnkle
    (0, 4), (4, 5), (5, 6),       # Pelvis - LHip - LKnee - LAnkle
...
def build_h36m17_adjacency(num_joints: int = 17) -> torch.Tensor:
...
def build_h36m17_partitioned_adjacency(num_joints: int = 17, center: int = 0) -> torch.Tensor:
    """Multi-partition adjacency for H3.6M-17, centred on Pelvis."""
...
class BinarySTGCNHead(nn.Module):
    """Multi-partition ST-GCN for binary classification on H3.6M-17 poses.

    Input  : (B, T, 51)  flattened 17 joints * 3 coords
    Output : (B, 2)      logits over [incorrect=0, correct=1]
    """
    ...
    def __init__(
        ...
        super().__init__()
        if n_features != num_joints * 3:
            raise ValueError(f"n_features={n_features} != {num_joints}*3")
        self.num_joints = num_joints
        self.register_buffer("A", build_h36m17_partitioned_adjacency(num_joints))

        channels = [3] + [hidden] * num_layers
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        B, T, _ = x.shape
        x = x.reshape(B, T, self.num_joints, 3).permute(0, 3, 1, 2)
